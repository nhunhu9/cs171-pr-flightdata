/* manually merged in zoomOnClick from https://github.com/markmarkoh/datamaps/pull/122/files */
(function() {
  var svg;
    ...
    if ( options.scope === 'usa' ) {
      projection = d3.geo.albersUsa()
       ...
       projection = d3.geo[options.projection]()
        ...
        .scale(2430)
        .translate([width / 2 - 289 + 2497 + 55 + 87, height / 2 - 2377 + 3540 - 238 + 46]);
    } else if ( options.scope === 'jpn' ) {
        projection = d3.geo[options.projection]()
        .center([66.166667, 34.4444])
        .scale(2430*0.43166977928720796*1.1991399137911205)
        .translate([width / 2-1055.4566615143528-250-64+21-169, height / 2 + 169.26318673172355-130-71+10+59]);
   } else if ( options.scope === 'vnm' ) {
  projection = d3.geo[options.projection]()
  .center([66.166667, 34.4444])
  .scale(2430*0.43166977928720796*1.546116449337655*1.0526311547094405*1.2379902908374518)
  .translate([width / 2-1055.4566615143528-250+711.0205122857296-610.96490112645-31-230, height / 2 + 169.26318673172355-130-784.5184106920008+164.63995089414033-105-63]);
  ...
  ***************************************/
  ...
  Datamap.prototype.getNameByCountryId = function(country_id) {
    var res = this.worldTopo.objects["world"].geometries.filter(function(d) { 
      return d.id == country_id });
...
;
  ...
  ***************************************/
  ...
  Datamap.prototype.updateScope = function(scope) {
    this.options.scope = scope.toLowerCase();
    this.draw();
}
  // expose library
